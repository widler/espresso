class << E
  ...
  module EViewSetup
    ...
    def view_path! path
      ...
      path =~ /\A\// ? absolute_view_path!(path) : @view__path = path.freeze
    end
    ...
    def absolute_view_path path
      @view__absolute_path || absolute_view_path!(path)
    end

    def absolute_view_path! path
      return if locked?
      @view__absolute_path = normalize_path(path.to_s << '/').sub(/\/+\Z/, '/').freeze
    end
